...
**NOTE**: *You can call your geonode project whatever you like following the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``geonode_dominode`` with whatever you would like to name your project.*
...
1. Prepare the Environment
    ...
    mkvirtualenv --python=/usr/bin/python3 geonode_dominode
    pip install Django==2.2.12

    django-admin startproject --template=./geonode-project -e py,sh,md,rst,json,yml,ini,env,sample,properties -n monitoring-cron -n Dockerfile geonode_dominode
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonode_dominode/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4geonode_dominode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonode_dominode/br/backup.sh $BKP_FOLDER_NAME'
```

### Run a Restore

```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonode_dominode/br/restore.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4geonode_dominode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonode_dominode/br/restore.sh $BKP_FOLDER_NAME'
```
